 $(document).ready(function(e) {
	...
	$('[data-open-layer]').on('click',function(event) { // when lcicked on locations in list, open a layer 
	...
	$('.closebtn').on('click',function(event) { // hide layer 
		
		var id = $(this).attr('data-record-id'); // location_1 
		
		$("#location_"+id).hide();
		
		$('[data-open-layer="location_'+id+'"]').parent().removeClass('openlayer');
	});
	...
	$("#stateDropdown li a").click(function(){
		...
		if(state_id === "all")
		{
			$( ".states" ).each(function( index ) { 
				...
				$(this).find('i').removeClass("fa-chevron-up").addClass("fa-chevron-down"); // fa-chevron-down 
		...
		{	
			...
			$( ".states" ).each(function( index ) { 
				...
				$(this).find('i').removeClass("fa-chevron-down").addClass("fa-chevron-up"); // fa-chevron-down 
				$(this).parent().parent().removeClass('isopen');
			});
			
			// open the selected
			var _section = $(".form_wrap").find("[data-form-section='" + state_id + "']");
	...
	// filter category 
	$('#categoryDropdown li a').on('click',function(event) { // dropdown category update map AJAX 
	...
	function update_map_markers(state_id, catgory_id, keyword)
	{
		...
		$.ajax({
			...
			success: function(data){
				...
				App.globals.locationMap = new google.maps.Map(document.getElementById("map_canvas"),myOptions);
				App.createMarkers();
				///
